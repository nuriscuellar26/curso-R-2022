...
* Las variables $\tau_i$ son variables aleatorias normales independientes, cada una con media 0 y varianza común $\sigma^2_{\tau}$.
...
\begin{tabular}{| c | c | }
\hline
Yacimientos & Porcentaje de azufre \\ \hline
...
5 & 80    90    124   82    72    57    118   54    130\\ \hline
\end{tabular}
\end{center}
...
```{r}
mod <- aov(Azufre ~ Yacimiento, data = porcentaje);mod
...
```{r}
hist(g, prob = TRUE, xlab ="Residuos", ylim = c(0, 0.025), main = 
...
```{r}
dias$Fotoperiodo = factor(dias$Fotoperiodo);dias$Fotoperiodo

dias$Genotipo = factor(dias$Genotipo);dias$Genotipo

# ANOVA
mod = aov(Dias ~ Fotoperiodo + Genotipo, data = dias);mod
...
```{r}
duncan1 <-duncan.test(mod,"Fotoperiodo",  main= 
                        "Número de días con diferentes fotoperiodos")
duncan1
```
...
```{r}
factorial$agua <- factor(factorial$Nivel_agua);factorial$agua

factorial$planta <- factor(factorial$Tipo_planta);factorial$planta
```

Para responder a este apartado debe resolverse el contraste de igualdad de medias para el factor A:
$H_0=\mu1=\mu2=\mu3=\mu$ vs $H_1=\mu i\ne\mu j$ para algún $i\ne j$

y para el factor B:
$H_0=\mu1=\mu2==\mu$ vs $H_1=\mu i\ne\mu j$ para algún $i\ne j$
...
$H0 = (\tau\beta)ij = 0$ (no existe interacción)   vs   $H1 = (\tau\beta)ij \ne 0$ (existe interacción).
...
175 & 21.9  & 22.1  & 21.85  &  21.9 &  \\ \hline
\end{tabular}
\end{center}
...
```{r}
propuesto1 <- read.table("s9_propuesto1.txt", header = TRUE)
propuesto1 
```

Transformando la variable referente a los niveles del factor fijo como factor 
...
```{r}
mod.tukey <- TukeyHSD(mod, ordered = TRUE);mod.tukey 
```

Contraste de comparaciones múltiples de Duncan

```{r}
duncan <-duncan.test(mod, "Temperatura" ,   
    main= " Modificaciones de la densidad de fluido al elevar la temperatura ")
...
B= 0.69212, y un p-valor = 0.8751, que nos indica que no se existe suficiente evidencia para rechazar la igualdad entre las varianzas entre las densidades registradas para las diferentes temperaturas
...
# Representamos el histograma
hist(g, prob = TRUE, xlab ="Residuos", ylim = c(0, 5), 
...
El gráfico Q-Q, no muestra desviaciones importantes a la normalidad

Comprobando la hipótesis de normalidad mediante el contraste de Shapiro-Wilk

```{r}
shapiro.test(mod$residuals)
```

El valor del p-valor es de 0.3712, por lo tanto no existe suficiente evidencia para rechazar la hipótesis de normalidad de los residuos

4.

El test de Duncan nos muestra:
```{r}
duncan
```
