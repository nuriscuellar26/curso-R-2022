...
\textbf{Ejemplo:}
En cierta colonia de San Salvador se selecciona aleatoriamente una muestra de 30 hogares, al medir
el número de hijos en cada unidad muestral se obtienen los siguientes datos:\\
...
<<>>=
hijos <- c(2, 1,2,1,4,2,3,0,2,3,3,2,1,0,2,4,
           1,2,1,3,4,1,2,3,1,5,2,3,1,2)
data.entry(hijos)
hijos
length(hijos)
@
...
\textbf{Observación: method puede ser:\\"overplot" (los puntos coincidentes son superpuestos)\\"jitter" (los puntos se ven como alejados o inquietos)\\"stack" (los puntos coincidentes son apilados, uno tras otro)}\\
...
#informe. Sin embargo, si estamos utilizando LATEX podemos utilizar la siguiente 
#instrucción xtable(tfre) y con esto nos genera el código correspondiente para 
#incorporarlo en nuestro archivo.
@
...
# na.rm = FALSE,le indica a R que los datos faltantes son omitidos en el cálculo
# de la media.
